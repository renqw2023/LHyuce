...
TOURNAMENT_SIZE = 5        # 锦标赛选择大小 (Number of individuals in a selection tournament)

# --- PARAMETER SPACE (The "Gene Pool") ---
PARAMETER_SPACE = {
    'trend_lookback': (3, 25),      # 分析趋势时回溯的期数
    'hot_score': (0.0, 2.0), 
    'cold_score': (0.0, 2.0), 
    'category_trend': (0.0, 2.0),
    'combo_2_diversity': (1.0, 1.5), 
    'combo_3_color_diversity': (1.0, 1.5),
    'combo_3_element_diversity': (1.0, 1.5),
    'co_occurrence_weight': (0.0, 5.0) # 新增基因：号码共现权重
}
...
def calculate_population_fitness(population, lottery_type, backtest_range):
    ...
    print(f"正在评估种群适应度 (共 {len(population)} 个个体)...")
    for i, individual in enumerate(population):
        ...
        print(f"  - 个体 {i+1}/{len(population)} 适应度: {fitness}")
...
def selection(population_with_fitness):
    ...
    # Return the individual with the highest fitness score in the tournament
...
def run_evolution(lottery_type, backtest_range):
    """Runs the main genetic algorithm evolution process."""
    print(f"--- 开始为 {lottery_type.upper()} 数据运行遗传算法优化 ---")
    ...
    for gen in range(N_GENERATIONS):
        ...
        # --- New: Log the best fitness of the generation ---
...
if __name__ == "__main__":
    # Run the evolution for both Hong Kong and Macau data to generate both strategy files.
